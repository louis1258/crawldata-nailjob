    ...
    while (retries < maxRetries) {
        try {
            ...
            if (retries < maxRetries) {
                ...
                return "fail"; // Tất cả retry đều thất bại
            }
        }
    }
}
...
connect({
    ...
    .then(async response => {
        ...
        await delay(10000)
        await page.goto("https://cmangal.com/album/bat-tu-va-bat-hanh-ban-mau-full-hd/chapter-121-1101922");
        ...
        channel.consume(queue, async (message) => {
            if (message !== null) {
                ...
                try {
                    await delay(8000);
                    const redirect = await gotoWithRetry(page, "https://cmangal.com/album/bat-tu-va-bat-hanh-ban-mau-full-hd/chapter-121-1101922", 3);
...
async function pagePromise(page , payload) {
    ...
    try {
        ...
        if (existingChapter) {
            console.log(`Chapter with order "${message.order}" and link "${payload.chapter}" already exists.`);
            await delay(2000)
